...
# Graphique A : Les groupes trouvés par le K-means (L'IA)
plt.subplot(1, 2, 1)
plt.scatter(X_pca[:, 0], X_pca[:, 1], c=clusters, cmap='viridis', alpha=0.6)
plt.title('Groupes trouvés par K-means (IA)')
plt.colorbar(label='Cluster ID')

# Graphique B : Les vraies opinions (La Modularité)
plt.subplot(1, 2, 2)
...
print("\n= RÉCAPITULATIF :")
print(pd.crosstab(y_true, clusters, rownames=['Réalité'], colnames=['Cluster IA']))